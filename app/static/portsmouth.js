...
const trainer=document.body.dataset.trainer||'portsmouth',trainerName=document.body.dataset.trainerName||'Portsmouth Gambit';
...
async function start(){moves=[];selected=null;complete=false;$('finish').classList.add('hidden');$('prompt').textContent='Find the repertoire move';$('feedback').textContent=`Start the ${trainerName} as White.`;state=await api('/api/state',{moves:[]});render();renderHistory()}
